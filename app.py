...
###INGRESO DE REQUEST DESDE WEB
    ...
    date_inicio=pd.Timestamp(date_inicio)

    ### valor fecha_fin
    date_fin=dfdata['date_fin'].values
    date_fin=date_fin[0]
    date_fin=pd.Timestamp(date_fin)

    ## edad
    edad=data_df2['edad'].values
    edad=edad[0]

    ## grado
    grado=data_df2['grado'].values
    grado=grado[0]

    ## estado
    estado=data_df2['estado'].values
    estado=estado[0]

    ## rubro
    rubro=data_df2['rubro'].values
    rubro=rubro[0]

    ## ec
    ec=data_df2['ec'].values
    ec=ec[0]

    ## eo
    eo=data_df2['eo'].values
    eo=eo[0]

    ## se
    se=data_df2['se'].values
    se=se[0]

    ## tipo
    tipo=data_df2['tipo'].values
    tipo=tipo[0]

    ## tiempo
    tiempo=data_df2['tiempo'].values
    tiempo=tiempo[0]
    tiempo
